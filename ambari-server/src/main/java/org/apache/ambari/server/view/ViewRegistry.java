...
// ----- ViewRegistry ------------------------------------------------------
  ...
  public void loadExtractedViews() {
    try {
      LOG.debug("Loading extracted views ...");
      File extractedViewsDir = new File(configuration.getViewsDir(), EXTRACTED_ARCHIVES_DIR);

      if (!extractedViewsDir.exists()) {
        LOG.info("There are no views extracted yet in the folder: {}", extractedViewsDir.getPath());
        return;
      }

      for (File viewDir : extractedViewsDir.listFiles()) {
        LOG.debug("Processing entry:[{}] from views dir.", viewDir.getPath());

        if (!viewDir.isDirectory()) {
          LOG.debug("The file [{}] is not a view root directory; skipping processing it.", viewDir);
          continue;
        }

        ViewConfig viewConfig = archiveUtility.getViewConfigFromExtractedViewDir(viewDir, false);
        final ViewEntity viewEntity = new ViewEntity(viewConfig, configuration, viewDir.getPath());

        addDefinition(viewEntity);

        readViewArchive(viewEntity, File.createTempFile("none", "none"), viewDir, ambariMetaInfoProvider.get().getServerVersion());
        ...
        for (ViewEntity view : getDefinitions()) {
          if (view.getStatus() == ViewDefinition.ViewStatus.DEPLOYED) {
            ...
            migrateDataFromPreviousVersion(view, ambariMetaInfoProvider.get().getServerVersion());
          }
        }
...
//        }
      ...
      LOG.warn("Could not load extracted views", e);
    }

    LOG.debug("Loading extracted views ... DONE.");
  }
  ...
  private void readViewArchive(ViewEntity viewDefinition,
    ...
    try {
      ...
      LOG.info("Get view config from extracted view folder: {}...", extractedArchiveDirPath);
      ...
      LOG.info("Get view config from extracted view folder: {} ... DONE", extractedArchiveDirPath);

      viewDefinition.setConfiguration(viewConfig);

      if (checkViewVersions(viewDefinition, serverVersion)) {
        LOG.info("Checked versions...");
        setupViewDefinition(viewDefinition, cl);
        LOG.info("Set up view def...");
        ...
        LOG.info("created view instance entities...");
        persistView(viewDefinition, instanceDefinitions);

        LOG.info("persisted  view instance entities...");
  ...
  private void configureViewLogging(ViewEntity viewDefinition, ClassLoader cl) {
    ...
    LOG.info("Marker: searching for {} ...", VIEW_LOG_FILE);
    InputStream viewLog4jStream = cl.getResourceAsStream(VIEW_LOG_FILE);
    LOG.info("Marker: searching for {} DONE.", VIEW_LOG_FILE);
    InputStream ambariLog4jStream = null;
    if (null != viewLog4jStream) {
      try {
        LOG.info("Marker: loading log 4 j {} ...", VIEW_LOG_FILE);
        ...
        LOG.info("Marker: loading  {} ...", AMBARI_LOG_FILE);
        ...
        LOG.info("Marker: configurator {} ...", viewLog4jConfig);
        PropertyConfigurator.configure(viewLog4jConfig);
        LOG.info("Marker: configurator {} DONE.", viewLog4jConfig);
      } catch (IOException e) {
