...
def get_truststore_path(properties, options):
  ...
  if not truststore_path:
    ...
    while not truststore_path:
      truststore_path = options.trust_store_path if options.trust_store_path is not None \
...
def get_truststore_type(properties, options):
  truststore_type = properties.get_property(SSL_TRUSTSTORE_TYPE_PROPERTY)
  if not truststore_type:
    SSL_TRUSTSTORE_TYPE_DEFAULT = get_value_from_properties(properties, SSL_TRUSTSTORE_TYPE_PROPERTY, "jks")
    truststore_type = options.trust_store_type if options.trust_store_type is not None \
      else get_validated_string_input(
...
def get_truststore_password(properties, options):
  ...
  if truststore_password:
    ...
    truststore_password = options.trust_store_password if options.trust_store_password is not None and options.trust_store_password \
      else read_password("", ".*", "Password for TrustStore:", "Invalid characters in password")
    if truststore_password:
...
def import_cert_and_key(security_server_keys_dir, options):
  import_cert_path = options.import_cert_path if options.import_cert_path is not None \
    else get_validated_filepath_input( \
      "Enter path to Certificate: ", \
      "Certificate not found")
  import_key_path  = options.import_key_path if options.import_key_path is not None \
    else get_validated_filepath_input( \
      "Enter path to Private Key: ", "Private Key not found")
  pem_password = options.pem_password if options.pem_password is not None and options.pem_password \
    else get_validated_string_input("Please enter password for Private Key: ", "", None, None, True)
...
def setup_https(options):
  ...
  if not get_silent():
    properties = get_ambari_properties()
    try:
      security_server_keys_dir = options.security_keys_dir if options.security_keys_dir is not None \
        else properties.get_property(SSL_KEY_DIR)
      client_api_ssl_port = None
      if options.client_api_ssl_port is not None:
        client_api_ssl_port = options.client_api_ssl_port
      else:
      ...
      api_ssl = options.api_ssl if options.api_ssl is not None else properties.get_property(SSL_API) in ['true']
      ...
      if api_ssl:
        ...
        if disable_https:
          ...
          ssl_api_port = options.client_api_ssl_port if options.client_api_ssl_port is not None else get_validated_string_input( \
            "SSL port ["+str(client_api_ssl_port)+"] ? ", \
            str(client_api_ssl_port), \
            "^[0-9]{1,5}$", "Invalid port.", False, validatorFunction = is_valid_https_port)
          properties.process_pair(SSL_API_PORT, ssl_api_port)
          cert_was_imported = import_cert_and_key_action(security_server_keys_dir, properties, options)
      else:
        configure_https = configure_https if configure_https is not None else get_YN_input("Do you want to configure HTTPS [y/n] (y)? ", True)
        if configure_https:
...
def setup_truststore(options, import_cert=False):
  if not get_silent():
    ...
    truststore_confirm = True if options.trust_store_path is not None and  options.trust_store_path \
      else get_YN_input("Do you want to configure a truststore [y/n] (y)? ", True)
    if truststore_confirm:
      truststore_type = get_truststore_type(properties, options)
      truststore_path = get_truststore_path(properties, options)
      truststore_password = get_truststore_password(properties, options)
